...
module.exports = function(config) {
	...
	return {
		configure: function(config) {
			db = mongo.db( (config? config : 'localhost:27017/paypal_pizza'), {w:1});
		},
		...
		updateUser: function(emailId, password, cardId, callback) {
					db.collection(USERS_COLLECTION).update( {email : emailId}, {email : emailId, password: encryptPassword(password), card: cardId}, function(err, result) {
						if(err) {
